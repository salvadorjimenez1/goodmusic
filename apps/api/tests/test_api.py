...
async def test_list_users_and_duplicate_register(client: AsyncClient):
    # Register a user
    reg = await client.post("/register", json={"username": "dupe_user", "password": "pw"})
    assert reg.status_code == 200

    # List users
    list_resp = await client.get("/users")
    assert list_resp.status_code == 200
    assert any(u["username"] == "dupe_user" for u in list_resp.json())

    # Try duplicate register
    dupe = await client.post("/register", json={"username": "dupe_user", "password": "pw"})
...
async def test_user_reviews_endpoint(client: AsyncClient, session: AsyncSession):
    ...
    assert resp.status_code == 200
    data = resp.json()
    assert any(r["album"]["title"] == "AlbumX" for r in data["reviews"])
